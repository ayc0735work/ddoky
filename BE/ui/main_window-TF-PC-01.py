...
class MainWindow(QMainWindow):
    ...
    def _on_mouse_input(self, mouse_info_str):
        """마우스 입력이 추가되었을 때 호출"""
        try:
            # 문자열을 딕셔너리로 변환
            import ast
            mouse_info = ast.literal_eval(mouse_info_str)
            
            # 로직 상세 위젯에 마우스 입력 추가
            self.logic_detail_widget.add_item(mouse_info['display_text'])
        except Exception as e:
            self._append_log(f"마우스 입력 처리 중 오류 발생: {str(e)}")
