...
class LogicDetailWidget(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.logic_manager = AllLogicDataRepositoryAndService(self.settings_manager)  # AllLogicDataRepositoryAndService 인스턴스 추가
    ...
    def _save_logic(self):
        ...
        try:
            ...
            self.log_message.emit(f"AllLogicDataRepositoryAndService.save_logic 호출 - ID: {self.current_logic_id}")
            print(f"[DEBUG] AllLogicDataRepositoryAndService.save_logic 호출 전 - ID: {self.current_logic_id}")
            # AllLogicDataRepositoryAndService를 통해 저장
            success, result = self.logic_manager.save_logic(self.current_logic_id, logic_info)
            self.log_message.emit(f"AllLogicDataRepositoryAndService.save_logic 결과: {success}, {result}")
            print(f"[DEBUG] AllLogicDataRepositoryAndService.save_logic 호출 후 - 결과: {success}, {result}")
