...
class CaptureOverlay(QDialog):
    ...
    def _capture_window(self):
        ...
        if not self.process_info or 'hwnd' not in self.process_info:
            self.base_log_manager.log(
                message="오류: 프로세스 정보가 없거나 잘못되었습니다.",
                level="ERROR",
                file_name="mouse_input_dialog",
                method_name="_capture_window",
        ...
        try:
            ...
            self.base_log_manager.log(
                message=f"창 정보 - 클라이언트 영역: ({self.client_width}x{self.client_height}), 위치: ({self.client_x}, {self.client_y})",
                level="DEBUG",
                file_name="mouse_input_dialog",
                method_name="_capture_window"
            )
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            self.base_log_manager.log(
                message=f"마우스 클릭 감지 - 상대좌표: ({relative_x}, {relative_y})",
                level="DEBUG",
                file_name="mouse_input_dialog",
                method_name="mousePressEvent"
            )
            self.base_log_manager.log(
                message=f"클라이언트 영역 크기: {self.client_width}x{self.client_height}",
                level="DEBUG",
                file_name="mouse_input_dialog",
                method_name="mousePressEvent"
            )
            self.base_log_manager.log(
                message=f"계산된 비율 - X: {x_ratio:.3f}, Y: {y_ratio:.3f}",
                level="DEBUG",
                file_name="mouse_input_dialog",
                method_name="mousePressEvent"
            )
            ...
            self.base_log_manager.log(
                message=f"클릭 정보 생성 완료: {self.click_info}",
                level="DEBUG",
                file_name="mouse_input_dialog",
                method_name="mousePressEvent"
            )
...
class MouseInputDialog(QDialog):
    ...
    def _select_coordinates(self):
        ...
        if not self.selected_process:
            self.base_log_manager.log(
                message="프로세스가 선택되지 않았습니다.",
                level="ERROR",
                file_name="mouse_input_dialog",
                method_name="_select_coordinates"
            )
            return
            
        self.base_log_manager.log(
            message=f"좌표 선택 시작 - 선택된 프로세스: {self.selected_process}",
            level="DEBUG",
            file_name="mouse_input_dialog",
            method_name="_select_coordinates"
        )
        ...
        if overlay.exec() == QDialog.Accepted:
            ...
            if click_info:
                ...
                self.base_log_manager.log(
                    message=f"좌표 정보 저장 시도 - X: {coordinates['x']}, Y: {coordinates['y']}",
                    level="DEBUG",
                    file_name="mouse_input_dialog",
                    method_name="_select_coordinates"
                )
                ...
                self.base_log_manager.log(
                    message=f"비율 정보 저장 시도 - X비율: {ratios['x']:.3f}, Y비율: {ratios['y']:.3f}",
                    level="DEBUG",
                    file_name="mouse_input_dialog",
                    method_name="_select_coordinates"
                )
    ...
    def _reset_coordinates(self):
        """좌표 입력 초기화"""
        self.base_log_manager.log(
            message="좌표 입력 초기화 시작",
            level="DEBUG",
            file_name="mouse_input_dialog",
            method_name="_reset_coordinates"
        )
        self.x_spinbox.setValue(0)
        self.y_spinbox.setValue(0)
        self.click_ratios = None
        self.base_log_manager.log(
            message="좌표 및 비율 정보 초기화 완료",
            level="DEBUG",
            file_name="mouse_input_dialog",
            method_name="_reset_coordinates"
        )
    ...
    def _on_confirm(self):
        ...
        self.base_log_manager.log(
            message=f"수집된 정보 - 동작: {action}, 버튼: {button}, 좌표: ({x}, {y}), 이름: {name}",
            level="DEBUG",
            file_name="mouse_input_dialog",
            method_name="_on_confirm"
        )
        self.base_log_manager.log(
            message=f"선택된 프로세스 정보: {self.selected_process}",
            level="DEBUG",
            file_name="mouse_input_dialog",
            method_name="_on_confirm"
        )
        ...
        if not hasattr(self, 'click_ratios') or self.click_ratios is None:
            ...
            self.base_log_manager.log(
                message=f"비율 정보: {self.click_ratios}",
                level="DEBUG",
                file_name="mouse_input_dialog",
                method_name="_on_confirm"
            )
        ...
        self.base_log_manager.log(
            message=f"생성된 마우스 입력 정보: {mouse_info}",
            level="DEBUG",
            file_name="mouse_input_dialog",
            method_name="_on_confirm"
        )
